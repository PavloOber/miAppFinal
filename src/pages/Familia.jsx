...
const Familia = () => {
  const { familiares, addFamiliar, deleteFamiliar, updateFamiliar, getFamiliarNameById } = useFamilia();
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'conyugeId') {
      setFormState(prevState => ({ ...prevState, [name]: value === '' ? null : parseInt(value, 10) }));
    } else {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        setFormState(prevState => ({ ...prevState, fotoBase64: reader.result }));
      };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formState.nombre || !formState.apellido || !formState.fechaNacimiento) {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {isFormVisible && (
        <form onSubmit={handleSubmit} className="mb-6 p-4 border rounded shadow-md grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50">
          ...
          <div>
             {/* Conyuge Selector */}
             <div className="mb-3">
                 <label htmlFor="conyugeId" className="block text-sm font-medium text-gray-700">Esposo/Esposa:</label>
                 <select 
                    id="conyugeId" 
                    name="conyugeId" 
             ...
             {/* ... (Selectors remain the same as previous version, but use otherFamiliares) ... */}
              <div className="mb-3">
                <label htmlFor="padresIds" className="block text-sm font-medium text-gray-700">Padre(s)/Madre(s) (Ctrl+Click):</label>
                <select 
                    id="padresIds" 
                    name="padresIds" 
                    multiple={true} 
             ...
             <div className="mb-3">
                <label htmlFor="hijosIds" className="block text-sm font-medium text-gray-700">Hijo(s)/Hija(s) (Ctrl+Click):</label>
                <select 
                    id="hijosIds" 
                    name="hijosIds" 
                    multiple={true} 
      ...
      {familiares.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {familiares.map((miembro) => (
            <div key={miembro.id} className="border p-4 rounded shadow relative bg-white">
    ...
    X
                ...
                <div className="flex items-center mb-2">
                    ...
                    {miembro.fotoBase64 ? (
                     ...
                     <div>
                         <h4 className="font-bold text-lg leading-tight">{miembro.nombre} {miembro.apellido}</h4>
                        {/* Display Conyuge if exists */}
                       {miembro.conyugeId && (
                            <p className="text-sm text-gray-600 leading-tight">Esposo/a: {getFamiliarNameById(miembro.conyugeId)}</p>
                       )}
                     </div>
                </div>

               <p className="text-sm text-gray-600">Nacimiento: {miembro.fechaNacimiento} (Edad: {calculateAge(miembro.fechaNacimiento)})</p>
               <p className="text-sm text-gray-600">Lugar: {miembro.ciudad ? `${miembro.ciudad}, ` : ''}{miembro.pais}</p>
                 ...
                 <div className="mt-2 pt-2 border-t">
                   ...
                   <ul className="list-disc list-inside text-sm">
                     {miembro.padresIds.map(parentId => (
                       <li key={parentId}>{getFamiliarNameById(parentId) || `ID: ${parentId}`}</li>
                     ))}
                   </ul>
                 </div>
               )}
                 ...
                 <div className="mt-2 pt-2 border-t">
                   ...
                   <ul className="list-disc list-inside text-sm">
                     {miembro.hijosIds.map(hijoId => (
                       <li key={hijoId}>{getFamiliarNameById(hijoId) || `ID: ${hijoId}`}</li>
                     ))}
                   </ul>
                 </div>
               )}
